...
class CSDNAdapter(SiteAdapter):
    """Adapter for CSDN blog articles."""

    def match(self, url: str) -> bool:
        try:
            host = urlparse(url).hostname or ""
            return host.endswith("csdn.net")
        except Exception:
            return False
    ...
    def site_config(self) -> SiteConfig:
        return SiteConfig(
            ...
            browser_wait_selector="#content_views",
            needs_browser=False,
        )

    def parse(self, html: str) -> Dict[str, Any]:
        title = ""
        m = re.search(r"<h1[^>]*class=\"[^\"]*title-article[^\"]*\"[^>]*>(.*?)</h1>", html, re.S | re.I)
        if m:
        ...
        author = ""
        m = re.search(r"class=\"[^\"]*follow-nickName[^\"]*\"[^>]*>(.*?)</", html, re.S | re.I)
        if m:
        ...
        content_html = ""
        m = re.search(r"<div[^>]*id=\"content_views\"[^>]*>(.*?)</div>\s*(?:<div|</article>)", html, re.S | re.I)
        if m:
